...
public class Blt extends InstructionB implements ILinkableInstruction {
    ...
    public static class Handler extends RiscV32InstructionHandler<Blt> {
        @Override
        public void handle(Blt instruction) throws IllegalRegisterException {
            instruction.exec(registerFile, programCounter);
        }
    }

    public static class Parser extends InstructionRegexParserRegisterBase<Blt> {
        @Override
        public Blt parse(String line) throws CompilationException {
            ...
            Blt instruction = new Blt(new InstructionBParams(
                (byte) rs1.getNumber(),
                (byte) rs2.getNumber(),
                (short) 0
            ));
